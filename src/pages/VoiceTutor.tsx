...
export default function VoiceTutor() {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    await streamChat({
      ...
      onDone: () => {
        setIsLoading(false);
        if (assistantSoFar) {
          speak(assistantSoFar.replace(/[#*`_]/g, "").slice(0, 500), speechLang);
        }
      },
      onError: (err) => {
        setIsLoading(false);
  ...
  }, [messages, language, speechLang, isLoading]);
    ...
    <div className="flex gap-4 h-[calc(100vh-120px)]">
      ...
      <div className="w-64 shrink-0 vs-card p-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-semibold">{t.topics}</h2>
        </div>
        <div className="space-y-1">
          {topicKeys.map((tk, i) => (
            ...
            >
              ...
              <span className={`rounded-full px-2 py-0.5 text-xs ${i === activeTopic ? "bg-primary-foreground/20" : "bg-muted"}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 && (
            <div className="flex h-full items-center justify-center text-center">
              <div>
                ...
                <p className="mt-4 text-lg font-medium text-muted-foreground">{t.startConversation}</p>
                <p className="text-sm text-muted-foreground/70">{t.askAnything}</p>
              </div>
            </div>
          )}
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className="max-w-[70%]">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`text-xs font-medium ${msg.role === "assistant" ? "text-primary" : "text-vs-purple"}`}>
                ...
                {msg.role === "assistant" && (
                  <div className="mt-1 flex gap-2">
                    ...
                    <button onClick={() => navigator.clipboard.writeText(msg.content)} className="text-muted-foreground hover:text-foreground">
                      <Copy className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => speak(msg.content.replace(/[#*`_]/g, ""), speechLang)} className="text-muted-foreground hover:text-foreground">
          ...
          {isLoading && messages[messages.length - 1]?.role !== "assistant" && (
            <div className="flex justify-start">
              <div className="rounded-2xl bg-muted px-4 py-3 text-sm animate-pulse">Thinking...</div>
            </div>
          )}
          {interimText && (
            <div className="flex justify-end">
              <div className="max-w-[70%] rounded-2xl bg-muted/50 px-4 py-3 text-sm italic text-muted-foreground">{interimText}</div>
            </div>
          )}
          <div ref={messagesEnd} />
        </div>
        ...
        <div className="border-t border-border p-4">
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex h-12 w-12 items-center justify-center rounded-xl vs-gradient-hero text-primary-foreground disabled:opacity-50"
            >
          ...
          <div className="mt-3 flex items-center gap-2 overflow-x-auto">
            ...
            {suggestions.map((s) => (
              <button key={s} onClick={() => sendMessage(s)} className="shrink-0 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium hover:bg-muted transition-colors">
                {s}
              </button>
            ))}
      ...
      <div className="w-64 shrink-0 vs-card p-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-semibold">{t.currentContext}</h2>
        </div>
        <div className="rounded-lg bg-muted/50 p-3 mb-4">
          <p className="text-xs text-muted-foreground">{t.activeTopic}</p>
          <p className="font-medium text-sm">{topicLabels[activeTopic]}</p>
        </div>
        <div className="mb-4">
          <p className="text-xs text-muted-foreground mb-2">{t.relatedLessons}</p>
          <p className="text-sm text-muted-foreground italic">{t.startConversation}</p>
        </div>
        <div>
          <p className="text-xs text-muted-foreground mb-2">{t.tryAsking}</p>
